...
def scatter_plot(tdclient, symbol, scatter, fig_size=None):
    ...
    if valid:
        ...
        def annote_adjuster(axes, pos):
            ...
            mid = 0.4
            high = 0.8
            mid_x = (xlims[1] - xlims[0]) * mid + xlims[0]
            high_x = (xlims[1] - xlims[0]) * high + xlims[0]

            if pos[0] >= mid_x:
                x = 0.1
            elif high_x > pos[0] >= mid_x:
                x = 0.18
            else:
                x = 0.15
            ...
            if scatter == "volume":
                ...
                text1 = "{}".format(
                    "\n".join(
                        str(call_symbol[n].split("_")[1])
                        + ": "
                        + "V/OI: {:,}, V: {:,}, OI: {:,}, bid: \${:,.2f}, ask: \${:,.2f}".format(
                            call_V_OI[n],
                            call_volumes[n],
                            call_interest[n],
                            call_bid[n],
                ...
                text1 = "{}".format(
                    "\n".join(
                        str(call_symbol[n].split("_")[1])
                        + ": "
                        + "BidAsk: \${:,.2f}, Vol: {:,}, bid: \${:,.2f}, ask: \${:,.2f}".format(
